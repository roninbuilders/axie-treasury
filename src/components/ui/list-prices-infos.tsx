				...
				<div className="p-7">
					<div className="mb-2">
						<div className="flex items-center mb-2">
							<span className="text-4xl font-bold text-gray-800 mr-2">5,037</span>
						...
						<span className="text-sm font-semibold text-gray-500">Treasury tokens</span>
					</div>
						...
						<div key={index}>
							<div className="flex items-center justify-between mb-4">
								<div className="flex items-center">
									<Image src={item.icon} alt={item.name} width={10} height={10} className="w-8 h-8 mr-4 rounded" />
									<div>
										<a href="#" className="text-gray-800 font-bold hover:text-blue-600">
											{item.name}
										</a>
										<span className="text-gray-500 text-sm block">{item.category}</span>
									</div>
								</div>
								<div className="flex items-center">
									<span className="text-gray-800 font-bold text-lg mr-3">{item.value}</span>
									<span
										className={`text-xs font-semibold inline-block py-1 px-2 rounded-full ${
											item.positive ? 'text-green-600 bg-green-200' : 'text-red-600 bg-red-200'
										}`}
									>
